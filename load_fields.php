...
$plan_arr = $conn->query("SELECT *,concat(weeks,' week/s [ ',interest_percentage,'%, ',penalty_rate,' ]') as plan FROM loan_plan where id  = '".$meta['plan_id']."' ")->fetch_array();
$weekly = ($meta['amount'] + ($meta['amount'] * ($plan_arr['interest_percentage']/100))) / $plan_arr['weeks'];
$penalty = $weekly * ($plan_arr['penalty_rate']/100);
