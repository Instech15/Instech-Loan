	...
	<form action="" id="loan-application">
		...
		<div class="row">
			<div class="col-md-6">
				...
				<?php
				$plan = $conn->query("SELECT * FROM loan_plan order by `weeks` desc ");
				?>
				<select name="plan_id" id="plan_id" class="custom-select browser-default select2">
					<option value=""></option>
						<?php while($row = $plan->fetch_assoc()): ?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($plan_id) && $plan_id == $row['id'] ? "selected" : '' ?> data-weeks="<?php echo $row['weeks'] ?>" data-interest_percentage="<?php echo $row['interest_percentage'] ?>" data-penalty_rate="<?php echo $row['penalty_rate'] ?>"><?php echo $row['weeks'] . ' week/s [ '.$row['interest_percentage'].'%, '.$row['penalty_rate'].'% ]' ?></option>
						<?php endwhile; ?>
				</select>
				<small>weeks [ interest%,penalty% ]</small>
			</div>
		...
		<div class="form-group col-md-6">
			<label class="control-label">Risk Premium</label>
			<input type="number" name="risk" class="form-control text-right" step="any" id="" value="<?php echo isset($risk) ? $risk : '' ?>">
		</div>

		<div class="form-group col-md-6">
			<label class="control-label">Registration Fee</label>
			<input type="number" name="reg" class="form-control text-right" step="any" id="" value="<?php echo isset($reg) ? $reg : '' ?>">
		</div>

		<div class="form-group col-md-6">
			<label class="control-label">Savings</label>
			<input type="number" name="savings" class="form-control text-right" step="any" id="" value="<?php echo isset($savings) ? $savings : '' ?>">
		</div>

		<div class="form-group col-md-6">
			<label class="control-label">Passbook Fee</label>
			<input type="number" name="pbook" class="form-control text-right" step="any" id="" value="<?php echo isset($pbook) ? $pbook : '' ?>">
		</div>

		<div class="form-group col-md-6">
			<label class="control-label">Loanform Fee</label>
			<input type="number" name="lform" class="form-control text-right" step="any" id="" value="<?php echo isset($lform) ? $lform : '' ?>">
		</div>

		<div class="form-group col-md-6">
			<label class="control-label">ID Card Fee</label>
			<input type="number" name="idcard" class="form-control text-right" step="any" id="" value="<?php echo isset($idcard) ? $idcard : '' ?>">
		</div>
...
<script>
	...
	function calculate(){
		...
		$.ajax({
			...
			method:"POST",
			data:{amount:$('[name="amount"]').val(),weeks:plan.attr('data-weeks'),interest:plan.attr('data-interest_percentage'),penalty:plan.attr('data-penalty_rate')},
